...
!--------------------------------------------------------------------
! HX added this subroutine
subroutine dos_tp (et, nspin, nbnd, nks, ntetra, tetra, e, dost, nat, proj_wght, pdos)
  !------------------------------------------------------------------
  !
  USE kinds, only : DP
  implicit none
  ...
  real(DP) :: et (nbnd, nks), e, dost (2)
  integer :: itetra (4), nk, ns, nt, ibnd, i, j

  real(DP) :: etetra (4), e1, e2, e3, e4
  integer :: nspin0

  integer :: nat,flag
  real(DP) :: proj_wght(nat, nbnd, nks), pdos(nat), tmp
  ...
  do ns = 1, nspin0
     ...
     do nt = 1, ntetra
        do ibnd = 1, nbnd
           ...
           flag = 1
           if (e.lt.e4.and.e.ge.e3) then
              tmp = 1.d0 / ntetra * (3.0d0 * (e4 - e) **2 / &
                   (e4 - e1) / (e4 - e2) / (e4 - e3) )
           elseif (e.lt.e3.and.e.ge.e2) then
              tmp = 1.d0 / ntetra / (e3 - e1) / (e4 - e1) &
                   * (3.0d0 * (e2 - e1) + 6.0d0 * (e-e2) - 3.0d0 * (e3 - e1 + e4 - e2) &
                   / (e3 - e2) / (e4 - e2) * (e-e2) **2)
           elseif (e.lt.e2.and.e.gt.e1) then
              tmp = 1.d0 / ntetra * 3.0d0 * (e-e1) **2 / &
                   (e2 - e1) / (e3 - e1) / (e4 - e1)
           else
              flag = 0
           endif
           if (flag.eq.1) then
               dost (ns) = dost (ns) + tmp
               ! fill PDOS
               ! assuming the four points on the tetragonal have the same weight
               do j = 1, nat
                   do i = 1, 1
                       pdos(j) = pdos(j) + proj_wght(j, ibnd, tetra (i, nt) + nk) * tmp
                   enddo
               enddo
               ! end of fill PDOS
           endif
