...
PROGRAM matdyn
  ...
  REAL(DP), ALLOCATABLE:: proj_wght(:,:,:), pdos(:) ! HX added
  ...
  IF (ionode) CALL input_from_file ( )
     ...
     IF (dos) THEN
       ...
       ALLOCATE( proj_wght(nat, 3*nat, nq) ) ! HX added
       ALLOCATE( pdos(nat) ) ! HX added
       proj_wght = 0.d0
     ELSE
        !
        ! read q-point list
...
!         WRITE(*,*) 'n = ',n
        do i = 1,3*nat
           do na = 1,nat
              nta = ityp(na)
              proj_wght(na,i,n) = &
              & REAL(DOT_PRODUCT(z((na-1)*3+1:(na-1)*3+3,i),&
              & z((na-1)*3+1:(na-1)*3+3,i))) * &
              & amu_ry * amass(nta)
!               WRITE(*,'(ES14.4)',advance='no') proj_wght(na,i,n)
           end do
!            proj_wght(:,i,n) = proj_wght(:,i,n) / sum(proj_wght(:,i,n))
!            WRITE(*,*) ''
        end do
        ! END OF THIS BLOCK
     ...
     IF (dos) THEN
        ...
        ! HX Changed this part
        WRITE (2, *) '# Energy (cm-1)  total DOS  PDOS(atom1) ... PDOS(atomN)'
        DO n= 1, ndos
           E = Emin + (n - 1) * DeltaE
!            CALL dos_t(freq, 1, 3*nat, nq, ntetra, tetra, E, DOSofE)
           CALL dos_tp(freq, 1, 3*nat, nq, ntetra, tetra, E, DOSofE, nat, proj_wght, pdos)
           !
...
!            IF (ionode) WRITE (2, '(ES12.4,ES12.4)') E, 0.5d0*DOSofE(1)
           IF (ionode) THEN
               WRITE (2, '(ES14.4,ES14.4)', advance='no') E, 0.5d0*DOSofE(1)
               do i=1,nat
                   WRITE (2, '(ES14.4)', advance='no') pdos(i)
               enddo
               WRITE (2, *) ''
           ENDIF
        END DO
     ...
     END IF
     DEALLOCATE ( freq)
     DEALLOCATE( proj_wght ) ! HX added
     DEALLOCATE( pdos ) ! HX added
